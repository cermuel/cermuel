...
export default function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  const { nodes: myNodes, materials: myMaterials } =
    useGLTF("/assets/3d/new.glb");

  const { nodes: dummyNodes, materials: dummyMaterials } = useGLTF(
    "/assets/3d/newcard.glb"
  );
  console.log({
    default: { nodes, materials },
    mine: { myNodes, myMaterials },
    new: { dummyNodes, dummyMaterials },
  });
  const texture = useTexture("/assets/images/cermuel.JPEG");
      ...
      <group position={[0, 4.6, 0]}>
        ...
        >
          ...
          >
            <mesh
              //@ts-ignore
              geometry={dummyNodes.Plane.geometry}
              scale={[0.4, 0.2, 0.6]}
              rotation={[Math.PI / 2, 0, 0]} // Rotates the plane to face the screen
              position={[0, 0.6, 0]} // Move the plane up by 0.5 units
            >
              <meshPhysicalMaterial
                // @ts-expect-error geometry/map are not declared?
                map={dummyMaterials.Back.map}
                map-anisotropy={16}
                clearcoat={1}
